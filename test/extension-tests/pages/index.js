...
import { getExpectedScreenshot, getPage, isExcluded, runGenericTests } from "./utils.js";
    ...
    driver.executeAsyncScript(async (...args) => {
      ...
      if (typeof browser != "undefined") {
        ...
        errors = await new Promise((resolve) =>
          chrome.runtime.sendMessage({ type: "filters.importRaw", text: filtersToAdd }, (errorsInResponse) =>
      ...
      if (typeof errors != "undefined" && errors[0] && errors[0].constructor === Array) {
        errors = errors[0];
      }
...
function removeFilters(driver, extensionHandle) {
  return runWithHandle(driver, extensionHandle, () =>
    driver.executeAsyncScript(async (...args) => {
      let callback = args[args.length - 1];
      // Firefox
      if (typeof browser !== "undefined") {
        let filters = await browser.runtime.sendMessage({
          type: "filters.get",
        });
        await Promise.all(
          filters.map((filter) =>
      ...
      if (typeof chrome != "undefined") {
        ...
        await Promise.all(
          filters.map(
            (filter) =>
              new Promise((resolve) => {
                chrome.runtime.sendMessage({ type: "filters.remove", text: filter.text }, (response) =>
                  resolve(response),
                );
              }),
          ),
        ...
        for (let [url, pageTitle] of testCases) {
          it(pageTitle, async function () {
            ...
            if (page in specializedTests) {
              ...
              let boundUpdateFilters = () => updateFilters(this.driver, this.extensionHandle, url);
              ...
              for (let element of await this.driver.findElements(locator)) {
                try {
                  await test.run(this.driver, element, this.extensionHandle, boundUpdateFilters);
                } catch (err) {
