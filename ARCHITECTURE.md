...
| `testext/` | The unpacked ABP extension used during test runs. Populated in one of two ways: (1) at runtime by `node ./test/extension-tests/extension-abp-snippets-download.js` (default on this branch, downloads the latest ABP build from the abp-snippets `next` branch; requires `ANTI_CV_TOKEN`), or (2) at Docker build time when the `EXTENSION_FILE` build arg is provided (`docker build --build-arg EXTENSION_FILE=...`). **Not committed — populated at runtime.** |
